...
export const getFragranceWithVendors = async (req, res) => {
  try {
    ...
    // Step 1: Get fragrance details + its products + its notes
    const fragrance = await Fragrance.findOne({
      where: { id: fragranceId },
      include: [
        {
        ...
        },
        {
          ...
          as: 'fragranceNotes',
          through: {
            attributes: ['noteType']  // includes noteType from join table
          }
        }
      ]
    });
    ...
    // Step 3: Get vendors offering this fragrance
    const vendorList = await VendorProduct.findAll({
      include: [
        {
          model: Product,
        ...
        },
        {
          model: BusinessInfo,
          as: 'company',
          attributes: ['id', 'companyName', 'image', 'websiteURL']
        }
      ],
      attributes: ['price', 'availability', 'purchaseLink', 'warranty']
    });
